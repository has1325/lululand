...
public class LululandService {
	...
	public String findUserId(String username, String email) {
		Optional<Lululand> user = lululandRepository.findByUsernameAndEmail(username, email.trim().toLowerCase());
	...
	public String findUserIdByNameAndPhone(String username, String phone) {
		...
		Optional<Lululand> user = lululandRepository.findByUsernameAndPhone(normalizedName, normalizedPhone);
	...
	public Lululand findByNameAndPhone(String username, String phone) {
		...
		return lululandRepository.findByUsernameAndPhone(normalizedName, normalizedPhone).orElse(null);
	}

	public Lululand getMetaloverByUseridAndUsernameAndEmail(String userid, String username, String email) {
		return lululandRepository.findByUseridAndUsernameAndEmail(userid, username, email)
				.orElseThrow(() -> new DataNotFoundException("Email not found!!"));
	}

	public Lululand findByEmail(String email) {
		return lululandRepository.findByEmail(email.trim().toLowerCase()).orElse(null);
	}
	...
	public void saveConsult(String name, String email, String jewelry, String message) {

		Consult consult = new Consult();

		consult.setName(name);
		consult.setEmail(email);
		consult.setJewelry(jewelry);
